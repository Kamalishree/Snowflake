...
df_cust = session.sql("select * from project_db.public.my_customer3")
df_cust.show()
...
df_customer = session.read.schema(schema).options({"field_delimiter":"|","skip_header":1}).csv('@snowpark')
df_customer.show(5)
df_customer.copy_into_table("customer_snowpark_aws_table")
df_customer.write.mode("append").save_as_table("orders_saved_table_appended",table_type="transient")